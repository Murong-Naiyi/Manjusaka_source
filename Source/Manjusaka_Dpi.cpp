...
const string DEFAULT_Dpi_CMD = "wm density | grep Physical | cut -d ':' -f2 | xargs echo";
const string CURRENT_APP_NAME_CMD = "am stack list|awk '/taskId/&&!/unknown/{print$2}'| awk -F: 'NR==1'|awk -F '/' '{print $1}'";
const string CURRENT_Dpi_CMD_1 = "wm density | grep Physical | cut -d ':' -f2 | xargs echo";
const string CURRENT_Dpi_CMD_2 = "wm density | grep Override | cut -d ':' -f2 | xargs echo";
ofstream log_f1(LOG_FILE_PATH.c_str());

// 获取默认Dpi
string get_default_Dpi()
{
    auto start_time = chrono::high_Dpi_clock::now(); // 开始计时
    FILE *fp = popen(DEFAULT_Dpi_CMD.c_str(), "r");
    if (fp == NULL)
    {
        std::string date_str = "";
    ...
    auto end_time = chrono::high_Dpi_clock::now(); // 结束计时
...
string get_current_Dpi()
{
    ...
    string current_res;
    {
        ifstream ifs(CURRENT_Dpi_CMD_1.c_str());
    ...
    {
        {
            ifstream ifs(CURRENT_Dpi_CMD_2.c_str());
            getline(ifs, current_res);
            ifs.close();
            current_res.erase(current_res.find_last_not_of("\n") + 1);
        }
...
int main()
{
    ...
    string default_res = get_default_Dpi();
    string prev_res;
    ...
    while (true)
    {
        auto loop_start_time = chrono::high_Dpi_clock::now(); // 记录每次循环开始的时间

        time_t cur_time = time(NULL);
        tm *timeinfo = gmtime(&cur_time);

        string current_app_name = get_current_app_name();
        string current_res = get_current_Dpi();
    ...
    if (FILE *fp = popen("date +\"%Y-%m-%d %H:%M:%S\"", "r")) {
       ...
       log_file << "[" << date_str << "]" << "Dpi已恢复到默认值: " << default_res << std::endl;
        ...
        auto loop_end_time = chrono::high_Dpi_clock::now(); // 记录每次循环结束的时间
